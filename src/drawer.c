...
scene *init_scene(grid *grid, bool full_blocks, bool edges, char **info, int infos_count)
{
...
static void draw_edges(const scene *scene)
{
	...
	{
		mvaddch(row, 0, VERTICAL_EDGE);
		mvaddch(row, scene->grid->cols + EDGE_OFFSET, VERTICAL_EDGE);
	}

	for (int col = 0; col < scene->grid->cols; col++)
	{
		mvaddch(0, col + EDGE_OFFSET, HORIZONTAL_EDGE);
		mvaddch(scene->grid->rows + EDGE_OFFSET, col + EDGE_OFFSET, HORIZONTAL_EDGE);
	}
...
static void draw_grid(const scene *scene)
{
	...
	{
		for (inr col = 0; col < scene->grid->cols; col++)
...
static void draw_block_if_different(const scene *scene, int row, int col)
{
        ...
        if ((future_block.content == (current_block & A_CHARTEXT)) &&
            (future_block.color == (current_block & A_COLOR))) {
                ...
                mvaddch(row + row_offset, col + col_offset, future_block.content);
